...
"""
These values are used to test against potential environment variable values that default to being true.
"""
_env_falsey_vals = ['n', 'no', 'false', '0']

"""
Various meta values used in the configuration.
name - Used to determine the name of the module and the folder the source code resides in.
backup_packages - Used iff find_packages fails or is unavailable.
readme - A readme containing a short and long description of the package.
version - Path to a python file that contains a line formatted as `__version_info__ = (#,#,#)`
authors - File containing the name, email, and website separated by newlines.
"""
...
try:
    if os.environ.get('PYTHON_USE_SETUPTOOLS', '').lower() in _env_falsey_vals: raise ImportError
...
def _load_authors(authors_file):
    """
    ...
    RETURNS
    -------
    Either the name, email, and website or 3 error strings. Always a tuple of size 3 strings.
    """
...
def readme(readme_file):
    ...
    PARAMETERS
    ----------
    readme_file - The name of the readme file. Do not give the absolute path.

    RETURNS
    -------
    The contents of the readme.
    """
    with open(os.path.join(os.path.dirname(__file__), readme_file), 'r') as f: return f.read()


def get_short_description(_readme):
    """
    ...
    """
    try:
        lines, idx = _readme.split('\n'), -1
        for i in range(len(lines)):
            if lines[i].startswith(name):
                idx = i+2 # 2 indexes forward because of rst formatting
                break
        if idx != -1: return lines[idx]
    except: pass
...
def get_version(version_file):
    ...
    PARAMETERS
    ----------
    version_file - The path to the file the version is located in.
    ...
    if mo:  vertup = mo.group(1), mo.group(2), mo.group(3)
    else: raise RuntimeError('Unable to find version string in %s.' % (version_file,))
    ver_str = '.'.join(vertup)
    return ver_str
